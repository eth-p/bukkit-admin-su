...
public class SuDisable implements CommandExecutor {

	private final Plugin adminsu;

	public SuDisable(Plugin adminsu) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length != 0) {
			adminsu.i18n().COMMAND_DISABLE_USAGE.param("command", label).send(sender);
			return true;
		}

		SuToggle.perform(adminsu, sender, b -> false);
		return true;
	}
